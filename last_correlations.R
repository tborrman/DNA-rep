...
raw_ori <-read.table(paste(begPath, "/Data/ori_data_1.7.txt", sep=""), header=TRUE, sep="\t", comment.char="");

# Find best correlation for n and ChIP seq

# Remove remaining origins in rDNA which skew ChIP-seq
ori_data <- raw_ori[-which(raw_ori$ID == 534),];

belsky_macalpine <- ori_data$rhind_belsky_MacAlpine.Upstream.MCM.Count + ori_data$rhind_belsky_MacAlpine.Downstream.MCM.Count;

data <- data.frame(ori_data$yang_n, ori_data$dsSPD4_MCM_count, ori_data$dsSPD4_MCM_no_mult, ori_data$dsSPD8_MCM_count, ori_data$dsSPD8_MCM_no_mult, belsky_macalpine, ori_data$macalpine_1_MCM_no_mult, ori_data$macalpine_2_MCM_no_mult);
cor_matrix <- cor(data, use="pairwise.complete.obs", method="pearson");
write.table(cor_matrix, paste(begPath, "/Results/MCM/MCM_corr_012714.txt", sep=""), sep='\t', row.names=TRUE, col.names=NA);

plot(ori_data$yang_n, ori_data$macalpine_2_MCM_no_mult, ylim = c(0,5000));
