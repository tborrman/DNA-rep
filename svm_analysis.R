...
# Read in ori_data_1.4.txt
full_ori_data <- read.table(paste(wkDir, "/ori_data_1.5.txt", sep=""), header=TRUE, sep="\t", comment.char="");
...
predict_values<- predict(svm_model, test_set$dsSPD4_MCM_count, probability=TRUE);
stop();
# Confusion matrix
...
for (kern_funct in functions) {
  ...
  # Compute ROC curves
  ROC_pred <- prediction(attr(predict_values_2D, "probabilities") [,1], test_set$class == "early" );
  ...
  profilePath <- paste(begPath, "/Results/svm/", kern_funct, "_svm_plot_new.pdf", sep="");
